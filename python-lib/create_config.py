...
def generate_key(project_key):
    ...
    process = Popen(['ssh-keygen', '-t', 'rsa','-b','2048','-f','rsa-key','-q','-N',"''"], stdout=PIPE, stderr=PIPE)
    gen_stdout, gen_stderr = process.communicte()#b'', b'' #process.stdout.readline(), process.stderr.readline()
